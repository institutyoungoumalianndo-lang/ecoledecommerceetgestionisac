...
import { type CreateTeacherInput, createTeacherInputSchema } from "@isac-erp/shared";
...
/** Création d'un enseignant (MODULE-05 §10.2) — page dédiée, formulaire trop dense pour une modale. */
export function TeacherFormScreen({
  ...
  const create = trpc.teachers.create.useMutation({ onSuccess: (teacher) => onCreated(teacher.id) });
  ...
  function onSubmit(values: CreateTeacherInput) {
    create.mutate({ ...values, photoPath });
  }

  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-foreground">Nouvel enseignant</h2>
      ...
      <form className="flex flex-col gap-6" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit" disabled={create.isPending}>
            {create.isPending ? "Enregistrement…" : "Créer l'enseignant"}
          </Button>
        </div>
      </form>
    </div>
