import type { StudentImportRowInput, ValidateStudentImportOutput } from "@isac-erp/shared";
...
type MappingField = keyof Omit<StudentImportRowInput, "rowNumber">;

const MAPPING_FIELDS: { key: MappingField; label: string; required: boolean }[] = [
  { key: "lastName", label: "Nom", required: true },
  { key: "firstName", label: "Prénom", required: true },
  { key: "gender", label: "Sexe", required: true },
  { key: "birthDate", label: "Date de naissance", required: false },
  { key: "birthPlace", label: "Lieu de naissance", required: false },
  { key: "nationality", label: "Nationalité", required: false },
  { key: "phonePrimary", label: "Téléphone principal", required: false },
  { key: "phoneSecondary", label: "Téléphone secondaire", required: false },
  { key: "email", label: "E-mail", required: false },
  { key: "maritalStatus", label: "Situation familiale", required: false },
...
const AUTO_MATCH: Record<MappingField, string[]> = {
  ...
  phonePrimary: ["téléphone", "telephone", "tel"],
  phoneSecondary: ["téléphone secondaire", "telephone secondaire"],
  email: ["email", "e-mail", "mail"],
  maritalStatus: ["situation familiale", "situation"],
  classCode: ["classe", "code classe"],
};

/** Assistant d'import Excel/CSV (MODULE-04 §4.11 / §3.8) — 3 étapes. */
export function StudentImportWizard({ onClose }: { onClose: () => void }) {
  ...
  const [validation, setValidation] = useState<ValidateStudentImportOutput | null>(null);
  ...
  const validate = trpc.studentImport.validate.useMutation();
  const execute = trpc.studentImport.execute.useMutation();
  ...
  function buildRows(): StudentImportRowInput[] {
    return rawRows.map((row, index) => ({
      rowNumber: index + 2, // ligne 1 = en-têtes
      lastName: mapping.lastName ? row[mapping.lastName] ?? "" : "",
      firstName: mapping.firstName ? row[mapping.firstName] ?? "" : "",
      gender: mapping.gender ? row[mapping.gender] ?? "" : "",
      birthDate: mapping.birthDate ? row[mapping.birthDate] : undefined,
      birthPlace: mapping.birthPlace ? row[mapping.birthPlace] : undefined,
      nationality: mapping.nationality ? row[mapping.nationality] : undefined,
      phonePrimary: mapping.phonePrimary ? row[mapping.phonePrimary] : undefined,
      phoneSecondary: mapping.phoneSecondary ? row[mapping.phoneSecondary] : undefined,
      email: mapping.email ? row[mapping.email] : undefined,
      maritalStatus: mapping.maritalStatus ? row[mapping.maritalStatus] : undefined,
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="flex max-h-[90vh] w-full max-w-3xl flex-col overflow-hidden rounded-lg border border-border window-surface text-window-foreground shadow-lg">
        <div className="flex items-center justify-between border-b border-border p-4">
          <h2 className="text-lg font-semibold text-foreground">Import d'étudiants — étape {step}/3</h2>
        ...
        <div className="flex-1 overflow-auto p-4">
                ...
                <table className="w-full text-sm">
                  <thead className="bg-secondary">
                    <tr>
                      ...
                      <th className="px-3 py-2 text-left text-secondary-foreground">Nom</th>
                  ...
                  <tbody>
                    {validation.results.map((r) => (
                      <tr key={r.rowNumber} className="border-t border-border">
                        ...
                        <td className="px-3 py-2">{r.data ? `${r.data.lastName} ${r.data.firstName}` : "—"}</td>
          ...
          {executeResult && (
            <Card variant="form">
              ...
              <CardContent>
                <p className="text-sm">{executeResult.importedCount} étudiant(s) importé(s).</p>
        ...
        <div className="flex justify-between border-t border-border p-4">
          ...
          <div className="flex gap-2">
            ...
            {step === 2 && (
              <Button
                disabled={!mapping.lastName || !mapping.firstName || !mapping.gender || validate.isPending}
            ...
            {step === 3 && !executeResult && validation && (
              <Button disabled={validation.validCount === 0 || execute.isPending} onClick={() => void handleExecute()}>
                {execute.isPending ? "Import…" : `Importer ${validation.validCount} étudiant(s)`}
              </Button>
            )}
