...
import { type BookCategoryDto, type CreateBookCategoryInput, createBookCategoryInputSchema } from "@isac-erp/shared";
...
/** Référentiel de catégories d'ouvrages (MODULE-13 §1.1) — aucune valeur codée en dur. */
export function BookCategoriesScreen() {
  const utils = trpc.useUtils();
  const canAdminister = useHasPermission("BIBLIOTHEQUE:ADMINISTRATION");
  const query = trpc.bookCategories.list.useQuery({});
  const [createOpen, setCreateOpen] = useState(false);

  function invalidate() {
    void utils.bookCategories.list.invalidate();
  }
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<CreateBookCategoryInput>({ resolver: zodResolver(createBookCategoryInputSchema) });

  const create = trpc.bookCategories.create.useMutation({
    onSuccess: () => {
      invalidate();
      setCreateOpen(false);
      reset();
  ...
  const update = trpc.bookCategories.update.useMutation({ onSuccess: invalidate });

  const columns: DataTableColumn<BookCategoryDto>[] = [
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-foreground">Catégories d'ouvrages</h2>
      ...
      <DataTable
        ...
        rows={query.data ?? []}
        getRowId={(c) => c.id}
        exportFilename="categories-ouvrages"
      ...
      <Dialog open={createOpen} onClose={() => setCreateOpen(false)} title="Créer une catégorie">
        <form className="flex flex-col gap-4" onSubmit={handleSubmit((values) => create.mutate(values))}>
          <FormField label="Nom" required error={errors.name?.message}>
            <Input {...register("name")} placeholder="Roman, Manuel scolaire…" />
          </FormField>
